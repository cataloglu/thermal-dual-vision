...
# Check HA_MODE to determine which mode to run
HA_MODE=${HA_MODE:-true}

if [ "$HA_MODE" = "false" ]; then
    # Standalone mode - skip bashio, use config.yaml and environment variables
    echo "Starting Smart Motion Detector in Standalone Mode..."
    echo "Configuration will be loaded from config.yaml and environment variables"

    cd /app
    exec python3 -m src.main
else
    # Home Assistant Add-on mode - use bashio for configuration
    bashio::log.info "Starting Smart Motion Detector in Home Assistant Add-on Mode..."
