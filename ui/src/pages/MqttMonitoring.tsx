...
export function MqttMonitoring() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-text mb-2">📡 {t('mqttMonitoringTitle')}</h1>
        <p className="text-muted">{t('mqttMonitoringDesc')}</p>
      </div>

      {!mqttStatus?.enabled ? (
        <div className="bg-surface1 border border-border rounded-lg p-12 text-center">
          <p className="text-muted mb-4">{t('mqttDisabled')}</p>
          <p className="text-sm text-muted">{t('mqttEnableHint')}</p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="bg-card p-6 rounded-lg border border-border">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-foreground">{t('connectionStatus')}</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {mqttStatus.connected_at && (
                <div className="p-3 bg-background rounded border border-border">
                  <div className="text-xs text-muted-foreground mb-1">{t('mqttConnectedAt')}</div>
                  <div className="text-sm text-foreground">
                    {new Date(mqttStatus.connected_at).toLocaleString(locale)}
                  </div>
                </div>
              )}
              <div className="p-3 bg-background rounded border border-border">
                <div className="text-xs text-muted-foreground mb-1">{t('mqttMessagesSent')}</div>
          ...
          <div className="bg-card p-6 rounded-lg border border-border">
            ...
            <div className="space-y-3">
              {cameras.length > 0 ? (
                ...
                <div className="text-muted-foreground text-center py-4">{t('mqttNoCameras')}</div>
              )}
            </div>
          ...
          <div className="bg-card p-6 rounded-lg border border-border">
            <h3 className="text-lg font-medium text-foreground mb-4">
              📤 {t('mqttActiveTopics')} ({mqttStatus.active_topics.length})
            </h3>
            <div className="max-h-64 overflow-y-auto space-y-1">
              {mqttStatus.active_topics.length > 0 ? (
                ...
                <div className="text-muted-foreground text-center py-4">{t('mqttNoTopics')}</div>
              )}
            </div>
          </div>
          ...
          <div className="bg-card p-6 rounded-lg border border-border">
            ...
            <div className="max-h-96 overflow-y-auto space-y-2">
              {Object.entries(mqttStatus.last_messages).length > 0 ? (
                ...
                <div className="text-muted-foreground text-center py-4">{t('mqttNoMessages')}</div>
              )}
            </div>
          </div>
          ...
          {mqttStatus.last_error && (
            <div className="bg-card p-6 rounded-lg border border-error">
              <div className="text-sm font-semibold text-error mb-2">{t('mqttLastError')}</div>
